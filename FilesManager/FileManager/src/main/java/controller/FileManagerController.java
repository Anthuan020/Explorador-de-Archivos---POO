...
public class FileManagerController {
    ...
    public FileManagerController(FileManager model, FileManagerView view) {
        ...
        this.view.addLogicButtonListener(new ButtonActionListener("Propiedades U.L."));
        this.view.addCopyButtonListener(new ButtonActionListener("Copiar"));
        this.view.addPasteButtonListener(new ButtonActionListener("Pegar"));
        ...
        public void actionPerformed(ActionEvent e) {
            switch (command) {
                ...
                case "Propiedades":
                    ...
                    break;
                case "Propiedades U.L.":
                    String fieInfo = view.getSelectedFile();
                    if (fieInfo != null) {
                        String logic = "C:\\";
                        view.setOutputText(model.getLogicProperties(logic));
                    } else {
                        String logic = "C:\\";
                        view.setOutputText(model.getLogicProperties(logic));   
                    }
                    break;
                case "Copiar":
                    String temp = view.getSelectedFile();
                    if (temp != null) {
                        origen = view.getPathInput() + "\\" + temp;
                        view.setOutputText("Archivo copiado con exito");
                    } else {
                        view.setOutputText("Seleccione un archivo o directorio para copiar.");
                    }
                    break;
                case "Pegar":
                {
                    try {
                        view.setOutputText("Ruta de origen: " +origen + "\n" + "Ruta de destino: " + view.getPathInput());
                        model.pasteElement(origen, view.getPathInput());
                        updateFileList(view.getPathInput());
                        view.setOutputText("Archivo pasteado con exito.");
            ...
            if (e.getClickCount() == 2) { // Doble clic para abrir carpeta
                ...
                if (selectedFile != null) {
                    ...
                    if (file.isDirectory()) {
                        ...
                        try{
                            view.setOutputText("Abriendo Archivo: " + newPath);
                            model.openFile(newPath);  
                        
                        } catch(FileNotFoundException r){
                            view.setOutputText("Error al abrir el archivo: " + newPath);
                        } catch (IOException ex) {
                            Logger.getLogger(FileManagerController.class.getName()).log(Level.SEVERE, null, ex);
                        }
